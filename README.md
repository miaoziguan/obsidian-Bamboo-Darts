...
- ✅ **灵活存储**：自定义目标文件夹、文件名模板
- ✅ **内容核查**：逐条比对原文，标记有据 / 存疑 / 无据，支持长文分段核查；同时检查数字、百分比、日期等数据准确性（内部比对 + AI 辅助验证）
...
| **Phase 5** | 内容核查 | 逐条比对原文，标记有据 / 存疑 / 无据，同时检查数字、百分比、日期等数据准确性 |
...
- ✅ **Content verification**: Cross-reference each note against source text; verify factual claims and numeric data (internal comparison + AI-assisted verification)
...
| **Phase 5** | Content Verification | Cross-reference each note against source text; verify factual claims and numeric data |
