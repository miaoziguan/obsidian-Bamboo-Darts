...
- ✅ **知识库去重**：与已有笔记按 TF-IDF 相似度比对，**严格只读取目标文件夹**，避免跨区域访问，可独立设置比对目录
...
| 启用知识库去重 | 提炼时与已有笔记比对，检测语义重复（仅读取目标文件夹） | 开启 |
| 去重目标文件夹 | 去重比对时的独立文件夹，留空则与"目标文件夹"相同 | 留空 |
| 标签词汇表 | 偏好标签，逗号或换行分隔 | — |
...
- ✅ **Vault deduplication**: Compares against existing notes using TF-IDF similarity; **strictly reads only from the configured folder**; comparison directory can be set independently
...
| Enable Vault Deduplication | Compare against existing notes during extraction; reads only from the configured folder | On |
| Dedup Target Folder | Separate folder for dedup comparison; leave empty to use "Target Folder" | Empty |
