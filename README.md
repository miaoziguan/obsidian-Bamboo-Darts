...
- ✅ **知识库去重**：与已有笔记按 TF-IDF 相似度比对，可独立指定去重目标文件夹（适合隐私需求用户限制去重范围）
...
- ✅ **Vault deduplication**: Compares against existing notes using TF-IDF similarity; a separate **dedup target folder** can be configured for privacy-conscious users
...
- **Vault dedup (Phase 4b)**: Compares against existing notes; the **dedup target folder** can be set separately from the save folder—ideal for privacy-conscious users who want to limit dedup scope
