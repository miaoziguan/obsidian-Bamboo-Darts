...
import { buildSimilarityMatrix, mmrRerank, NoteMeta, invalidateDiscoveryCache } from '../discovery/similarity-matrix';
...
export class AtomicNotesPanel extends ItemView {
  ...
  private _confirmingDelBtn: { el: HTMLElement; timeout: ReturnType<typeof setTimeout> } | null =
    null;
  ...
  private renderHistoryPanel(el: HTMLElement): void {
    ...
    clearBtn.addEventListener('click', async () => {
      ...
      const confirmModal = new (class extends Modal {
        ...
        constructor(app: App, parent: AtomicNotesPanel, targetEl: HTMLElement) {
          super(app);
          this.parent = parent;
      ...
      if (entry.savedPaths && entry.savedPaths.length > 0) {
        for (const savedPath of entry.savedPaths) {
          const linkEl = itemEl.createEl('a', {
            ...
            attr: {
              href: '#',
              style: 'font-size:11px;color:var(--text-accent);display:block;margin-left:8px',
            },
          });
  ...
  private renderRecommendation(container: HTMLElement): void {
    ...
    const runSimilarity = async () => {
      ...
      try {
        ...
        const simToQuery: number[] = matrix[idx];

        const ranked = mmrRerank(simToQuery, matrix, idx, topK, mmrLambda);
